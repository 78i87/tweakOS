...
export default function TerminalIcon() {
  ...
  const [app, setApp] = useState(() => getApp('terminal'));
  const [position, setPosition] = useState({ x: 120, y: 32 });
  const didDragRef = useRef(false);
  const dragStartPositionRef = useRef({ x: 0, y: 0 });

  useEffect(() => {
    const unsubscribe = subscribeToApps(() => {
      setApp(getApp('terminal'));
    });
    return unsubscribe;
  }, []);
  ...
  const handleClick = () => {
    ...
    if (terminalWindow) {
      if (terminalWindow.status === 'minimized') {
        restoreWindow(terminalWindow.id);
      }
